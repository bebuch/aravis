...
namespace tools{
	...
	namespace impl{ namespace log{
		...
		template < typename L > std::ostringstream& get_os(L& log);
		template < typename L > std::size_t get_id(L& log);
	...
	private:
		...
		template < typename L > friend std::ostringstream& impl::log::get_os(L& log);
		template < typename L > friend std::size_t impl::log::get_id(L& log);

		friend void set_os(Log& log, std::ostringstream& os){
			log.os = &os;
		}

		friend void set_body(Log& log){
			log.has_body = true;
		}
	};

	template < typename Log, typename T >
	inline log_base< Log >& operator<<(log_base< Log >& log, T&& v){
	...
	namespace impl{ namespace log{
		...
		template < typename L >
		inline std::ostringstream& get_os(L& log){
			return *log.os;
		}

		template < typename L >
		inline std::size_t get_id(L& log){
			return log.id;
		}

		template < typename Log >
		inline void do_init(Log& log){
		...
		template < typename Log >
		void check_log_base(){
			static_assert(std::is_base_of< log_base< Log >, Log >::value, "type Log have to be derived from log_base< Log >");
		}
